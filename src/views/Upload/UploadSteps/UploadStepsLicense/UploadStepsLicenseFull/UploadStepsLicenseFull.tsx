...
export default function UploadStepsLicenseFull(props: IProps) {
	...
	React.useEffect(() => {
		if (!props.hasLicense) dispatch(uploadActions.setStepDisabled(false));
		else {
			if (
			...
			<S.FlexWrapper>
				...
				<S.IWrapper disabled={!accessFee || accessFee.id === concatLicenseTag(licenseParams.access.options.none)}>
					<FormField
						...
						disabled={
							!accessFee || accessFee.id === concatLicenseTag(licenseParams.access.options.none) || !props.hasLicense
						}
						hideErrorMessage
					/>
					<ReactSVG src={ASSETS.wrappedAr} />
